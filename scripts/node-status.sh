...
DASH_PORT="${DASH_DAEMON_SERVICE_PORT_RPC:-9998}"
...
if [ "$VALIDATOR" = "true" ]; then
  ...
  if [ -n "$DASH_DAEMON_SERVICE_PORT_RPC" ]; then
    DASH_RESULT=$(curl -sL --fail -m 10 --data-binary '{"jsonrpc": "1.0", "id": "node-status", "method": "getblockchaininfo", "params": []}' -H 'content-type: text/plain;' http://thorchain:password@dash-daemon:"$DASH_PORT")
...
[ "$VALIDATOR" = "true" ] && printf "%-11s %-10s %-10s\n" DASH "$DASH_PROGRESS" "$(format_int "$DASH_SYNC_HEIGHT")/$(format_int "$DASH_HEIGHT")"
