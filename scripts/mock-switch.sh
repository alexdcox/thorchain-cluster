...
while true; do
  INBOUND_ADDRESS=$(curl -s "$4:1317/thorchain/inbound_addresses" 2>/dev/null | jq -r '.[]|select(.chain=="BNB") .address' 2>/dev/null)
  if [[ $INBOUND_ADDRESS != "" ]]; then
    echo "INBOUND_ADDRESS $INBOUND_ADDRESS"
    break
  fi
done
